...
```{r Support Transcript Models, echo=FALSE}
suppressWarnings(
  suppressMessages(
    # Generate figure 2a
fig_2a_challenge1(new_data = c(temp_env$all.results$global$Value[2],
                               temp_env$all.results$global$Value[5],
                               temp_env$all.results$global$Value[5] - temp_env$all.results$global_SJ$`Absolute value`[2],
                               temp_env$all.results$global_SJ$`Absolute value`[2],
                               temp_env$all.results$global$Value[5] - temp_env$all.results$global_SJ$`Absolute value`[1],
                               temp_env$all.results$global_SJ$`Absolute value`[1],
                               pipelineCode,
                               temp_env$all.results$global$Value[3],
                               sum('FSM' == temp_env$sqanti_data$structural_category),
                               sum('ISM' == temp_env$sqanti_data$structural_category),
                               sum('NIC' == temp_env$sqanti_data$structural_category),
                               sum('NNC' == temp_env$sqanti_data$structural_category),
                               sum('Antisense' == temp_env$sqanti_data$structural_category),
                               sum('Fusion' == temp_env$sqanti_data$structural_category),
                               sum('Genic_Genomic' == temp_env$sqanti_data$structural_category),
                               sum('Genic_Intron' == temp_env$sqanti_data$structural_category),
                               sum('Intergenic' == temp_env$sqanti_data$structural_category),
                               sum(temp_env$all.results$global$Value[5] + temp_env$all.results$global_SJ$`Absolute value`[1] + temp_env$all.results$global_SJ$`Absolute value`[2]),
                               sum(temp_env$all.results$global$Value[5] + temp_env$all.results$global_SJ$`Absolute value`[1] + temp_env$all.results$global_SJ$`Absolute value`[2]) - temp_env$all.results$global_SJ$`Absolute value`[1],
                               temp_env$all.results$global_SJ$`Absolute value`[1],
                               sum(temp_env$all.results$global$Value[5] + temp_env$all.results$global_SJ$`Absolute value`[1] + temp_env$all.results$global_SJ$`Absolute value`[2]) - temp_env$all.results$global_SJ$`Absolute value`[2],
                               sum(temp_env$sqanti_data.junc$RTS_junction == 'TRUE'),
                               sum(temp_env$all.results$global$Value[5] + temp_env$all.results$global_SJ$`Absolute value`[1] + temp_env$all.results$global_SJ$`Absolute value`[2]) - sum(temp_env$sqanti_data.junc$sample_with_cov == 0)),
                  if (dataset2) {
                    new_data2 <- c(
                      temp_env2$all.results$global$Value[2],
                      temp_env2$all.results$global$Value[5],
                      temp_env2$all.results$global$Value[5] - temp_env2$all.results$global_SJ$`Absolute value`[2],
                      temp_env2$all.results$global_SJ$`Absolute value`[2],
                      temp_env2$all.results$global$Value[5] - temp_env2$all.results$global_SJ$`Absolute value`[1],
                      temp_env2$all.results$global_SJ$`Absolute value`[1],
                      pipelineCode2,
                      temp_env2$all.results$global$Value[3],
                      sum('FSM' == temp_env2$sqanti_data$structural_category),
                      sum('ISM' == temp_env2$sqanti_data$structural_category),
                      sum('NIC' == temp_env2$sqanti_data$structural_category),
                      sum('NNC' == temp_env2$sqanti_data$structural_category),
                      sum('Antisense' == temp_env2$sqanti_data$structural_category),
                      sum('Fusion' == temp_env2$sqanti_data$structural_category),
                      sum('Genic_Genomic' == temp_env2$sqanti_data$structural_category),
                      sum('Genic_Intron' == temp_env2$sqanti_data$structural_category),
                      sum('Intergenic' == temp_env2$sqanti_data$structural_category),
                      sum(temp_env2$all.results$global$Value[5] + temp_env2$all.results$global_SJ$`Absolute value`[1] + temp_env2$all.results$global_SJ$`Absolute value`[2]),
                      sum(temp_env2$all.results$global$Value[5] + temp_env2$all.results$global_SJ$`Absolute value`[1] + temp_env2$all.results$global_SJ$`Absolute value`[2]) - temp_env2$all.results$global_SJ$`Absolute value`[1],
                      temp_env2$all.results$global_SJ$`Absolute value`[1],
                      sum(temp_env2$all.results$global$Value[5] + temp_env2$all.results$global_SJ$`Absolute value`[1] + temp_env2$all.results$global_SJ$`Absolute value`[2]) - temp_env2$all.results$global_SJ$`Absolute value`[2],
                      sum(temp_env2$sqanti_data.junc$RTS_junction == 'TRUE'),
                      sum(temp_env2$all.results$global$Value[5] + temp_env2$all.results$global_SJ$`Absolute value`[1] + temp_env2$all.results$global_SJ$`Absolute value`[2]) - sum(temp_env2$sqanti_data.junc$sample_with_cov == 0))
                    
                  } else {
                    new_data2 <- NULL
                  },
                  if (organism == 'human') {data_sample = "WTC11"} else if (organism == 'mouse') {data_sample = "ES"},
                  outdir = "./",
...
```
#### Spike-In Based Performance
```{r Spike-In Based Performance, echo=FALSE}
# Generate figure 2c
suppressMessages(
  fig_2c_challenge1(if(organism == 'human'){data_sample = "WTC11"} else if(organism == 'mouse'){data_sample = "ES"}, 
                  outdir = outdir)
)
```
