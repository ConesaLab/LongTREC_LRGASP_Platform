...
def get_class_junc_filenames(args, dir=None):
    ...
    outputPathPrefix = os.path.join(os.getcwd(), args.output)
...
def run(args):
    ...
    for i in range(1, loop + 1):
        if i == 1:
            current_loop = 1
            args.isoforms = args.isoforms1
            args.platform = args.platform1
            args.lib_prep = args.lib_prep1
            args.data_cat = args.data_cat1
            args.dir = os.getcwd()
            ...
            if not os.path.isfile(args.ref_2):
                ...
                sys.exit()
            print('USED args.genome2:', args.ref_2)
            ...
            args.lib_prep = args.lib_prep2
            args.data_cat = args.data_cat2
            outputClassPath, outputJuncPath = get_class_junc_filenames(args)
        ...
        if i == 1:
            ...
            busco_tsv2 = run_BUSCO(corrFASTA, args.cpus, os.getcwd())

        print('The files used are: args.isoforms:', args.isoforms, '; args.genome:', args.genome, '; args.annotation:',
              args.annotation)
    ...
    if not args.skip_report:
        ...
        if args.sirv_list != 'NA':
            args.sirv_list = os.path.join('../../', args.sirv_list)
        if args.ercc_list != 'NA':
            args.ercc_list = os.path.join('../../', args.ercc_list)
        if args.sequin_list != 'NA':
            args.sequin_list = os.path.join('../../', args.sequin_list)
        if args.sirv_list2 != 'NA':
            args.sirv_list2 = os.path.join('../../', args.sirv_list2)
        if args.ercc_list2 != 'NA':
            args.ercc_list2 = os.path.join('../../', args.ercc_list2)
        if args.sequin_list2 != 'NA':
            args.sequin_list2 = os.path.join('../../', args.sequin_list2)
        ...
        cmd = RSCRIPTPATH + " {d}/{f} {c} {j} {d} {o} {b} {s} {t}your_tool {pl} {l} {da} {comp} {bambu} {RNABloom} {rnaSPAdes} {StringTie2IsoQuant} {sirv_list} {ercc_list} {sequin_list} {c2} {j2} {b2} {pl2} {l2} {da2} {sirv_list2} {ercc_list2} {sequin_list2} {dataset2}".format(
            ...
            RNABloom=args.comp_RNA_Bloom,
            rnaSPAdes=args.comp_rnaSPAdes,
            StringTie2IsoQuant=args.comp_StringTie2_IsoQuant,
...
def main():
    ...
    parser.add_argument('organism',
                        help='\t\tOrganism Transcriptome -> used to determine corresponding reference Transcriptome')
    parser.add_argument('platform',
                        help='\tPlatform that was used for the generation of the FASTA/FASTQ file. Default = PacBio')
    ...
    parser.add_argument('coverage_dir', help="\t ?")
    parser.add_argument('isoforms2',
                        help='\tIsoforms (FASTA/FASTQ) or GTF format. Recommend provide GTF format with the --gtf option.')
    parser.add_argument('platform2',
                        help='\tPlatform that was used for the generation of the FASTA/FASTQ file. Default = PacBio')
    parser.add_argument('lib_prep2', help="\tThe library preparation for the sample. Default = cDNA.")
    parser.add_argument('data_cat2', help="\t ?")
    parser.add_argument('anno_2', help="\t ?")
    parser.add_argument('ref_2', help="\t ?")
    parser.add_argument('cov_2', help="\t ?")
    parser.add_argument('cov_dir_2', help="\t ?")
    parser.add_argument('comparison', help="\t ?")
    parser.add_argument('comp_Bambu', help="\t ?")
    parser.add_argument('comp_RNA_Bloom', help="\t ?")
    parser.add_argument('comp_rnaSPAdes', help="\t ?")
    parser.add_argument('comp_StringTie2_IsoQuant', help="\t ?")
    ...
    parser.add_argument("--skipORF", default=False, action="store_true",
    ...
    parser.add_argument('-g', '--gtf', help='\t\tUse when running SQANTI by using as input a gtf of isoforms',
                        action='store_true')
    parser.add_argument('-e', '--expression', help='\t\tExpression matrix (supported: Kallisto tsv)',
                        required=False)
    ...
    if args.gff3 is not None:
        ...
        if not os.path.isfile(args.gff3):
            print("ERROR: Precomputed tappAS GFF3 annotation file {0} doesn't exist. Abort!".format(args.genome),
                  file=sys.stderr)
            sys.exit(-1)
    ...
    if not args.gtf:
        ...
        print("Cleaned up isoformmm fasta file written to: {0}".format(args.isoforms), file=sys.stderr)
    ...
    if args.dataset2 != 'NA':
        ...
        print("Cleaned up isoform fasta file written to: {0}".format(args.isoforms2), file=sys.stderr)
    ...
    if args.dataset2 != 'NA':
        ...
        with open(args.doc, 'w') as f:
            ...
            f.write("FLCount\t" + (os.path.basename(args.fl_count) if args.fl_count is not None else "NA") + "\n")
            f.write(
                "Expression\t" + (os.path.basename(args.expression) if args.expression is not None else "NA") + "\n")
            f.write("Junction\t" + (os.path.basename(args.coverage) if args.coverage is not None else "NA") + "\n")
            f.write("CagePeak\t" + (os.path.basename(args.cage_peak) if args.cage_peak is not None else "NA") + "\n")
            f.write(
                "PolyA\t" + (
                    os.path.basename(args.polyA_motif_list) if args.polyA_motif_list is not None else "NA") + "\n")
            f.write("PolyAPeak\t" + (os.path.basename(args.polyA_peak) if args.polyA_peak is not None else "NA") + "\n")
            f.write("IsFusion\t" + str(args.is_fusion) + "\n")
